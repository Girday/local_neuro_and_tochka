...
## Микросервис: **Safety Service (Input/Output Guard)**
...
- централизованное управление safety-политиками.
...
- Каркас кода расположен в `services/safety_service`.
- Экспонирует эндпоинты `/health`, `/internal/safety/input-check`, `/internal/safety/output-check` (FastAPI).
- Конфигурация через переменные окружения `SAFETY_SERVICE_*` (`config.py`), включая режим политики (`strict/balanced/relaxed`) и блок-лист.
- Логика безопасности реализована в `core/evaluator.py`: простые эвристики (блок-листы, маркеры prompt injection, PII regex) с режимами `blocked/transformed/allowed`.
- Автотесты находятся в `services/safety_service/tests` и покрывают ядро и HTTP-эндпоинты.
...
1. **Классификация входящих запросов**:
   - вредоносные запросы (взлом, вредоносное ПО, эксплуатация уязвимостей),
...
5. **Конфигурируемые политики**:
   ...
   - уровни строгости (strict / balanced / relaxed),
...
Проверка входящего пользовательского запроса до выполнения Retrieval/LLM.
...
- `blocked` — возвращают стандартизированный отказ (safe refusal).
...
2. **Data Exfiltration / Leakage**
   - попытки извлечь секреты, токены, конфиги, внутреннюю документацию других tenants.

3. **PII / Sensitive Data**
   - персональные данные, учётные данные, пароли, токены.
...
7. **Prompt Injection / Jailbreak Attempts**
   - попытки изменить инструкции модели, отключить безопасность, получить system prompt.
...
- `relaxed` — минимальные ограничения (только самые серьёзные нарушения).

### 5.2.2 Структура политики (логическая модель)
...
Хранилище политик может находиться:
...
2. **Policy Engine**
   - Загрузка и кэширование политик (per tenant / global).
...
## 7.4 Наблюдаемость (Observability)
